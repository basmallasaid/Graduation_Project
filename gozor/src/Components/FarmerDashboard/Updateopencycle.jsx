...
export default function Updateopencycle({ selectedCardData, onUpdateSuccess }) {
  ...
  const [parcelId, setParcelId] = useState(null); // Added to store parcel ID
  const [selectedProductType, setSelectedProductType] = useState("");
  const [selectedCropId, setSelectedCropId] = useState(null); // New: Store crop ID
  ...
  useEffect(() => {
    if (selectedCardData) {
      ...
      setParcelId(selectedCardData.parcelId || null); // Ensure parcelId is always a number
      ...
      setExpectedYield(selectedCardData.expectedYield?.toString() || "");
       setOpenInvestmentCycleDTO({
        expectedFinancialGoal:
          selectedCardData.openInvestmentCycleDTO?.expectedFinancialGoal?.toString() ||
          "",
        minimumInvestment:
          selectedCardData.openInvestmentCycleDTO?.minimumInvestment?.toString() ||
          "",
        maximumInvestment:
          selectedCardData.openInvestmentCycleDTO?.maximumInvestment?.toString() ||
          "",
        maxInvestorsAllowed:
          selectedCardData.openInvestmentCycleDTO?.maxInvestorsAllowed?.toString() ||
          "",
        availableProfitTypes:
          selectedCardData.openInvestmentCycleDTO?.availableProfitTypes || "",
      });
  ...
  const validateForm = () => {
   ...
   if (!openInvestmentCycleDTO.minimumInvestment || !openInvestmentCycleDTO.maximumInvestment) {
    ...
    if (
      Number(openInvestmentCycleDTO.minimumInvestment) >
      Number(openInvestmentCycleDTO.maximumInvestment)
    ) {
      toast.error("الحد الأدنى للاستثمار لا يمكن أن يكون أكبر من الحد الأقصى");
      return false;
    }

    if (!openInvestmentCycleDTO.availableProfitTypes) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateForm() || !selectedCardData?.id) {
     ...
     const data = {
      cycleName,
      parcelName,
      parcelId, // Include parcel ID
      selectedProductType,
      cropId: selectedCropId, // Include crop ID
      startDate,
      endDate,
    ...
    try {
      const response = await axios.put(
        `http://localhost:8000/viewyield/${selectedCardData.id}`,
        data
      );
