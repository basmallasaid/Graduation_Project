...
export default function Updateopencycle({ selectedCardData, onUpdateSuccess }) {
  ...
  useEffect(() => {
    if (selectedCardData) {
      ...
      setSelectedProductType(selectedCardData.cropName || selectedCardData.selectedProductType || "");
      ...
      setUpdateOpenInvestmentCycleDTO(
        selectedCardData.openInvestmentCycleDTO
          ? {
              ...
              openInvestmentCycleId: selectedCardData.openInvestmentCycleDTO.openInvestmentCycleId || 0,
            }
          : {
              expectedFinancialGoal: "",
              minimumInvestment: "",
   ...
   useEffect(() => {
     ...
     console.log("the seeds in use effect",seeds);
  ...
  const validateForm = () => {
    ...
    if (isOpenForInvestmentState) {
      ...
      if (Number(updateOpenInvestmentCycleDTO.minimumInvestment) > Number(updateOpenInvestmentCycleDTO.maximumInvestment)) {
  ...
  const handleSubmit = async (e) => {
    ...
    const data = {
      ...
      updateOpenInvestmentCycleDTO: {
        openInvestmentCycleId: updateOpenInvestmentCycleDTO.openInvestmentCycleId,
    ...
    try {
      const response = await api.put("Cycle/EditCycle", data);
  ...
  const inputStyle = { borderRadius: "10px", borderColor: "#1F4E3D" };

  const renderCropSelect = (crops, placeholder) => (
    <select
    ...
    >
      ...
      {crops.map((crop) => (
        <option key={crop.id} value={JSON.stringify(crop)}>
  ...
  return (
    <div className="container d-flex justify-content-center align-items-center">
      <ToastContainer
        ...
        theme="colored"
      />
      <form onSubmit={handleSubmit} className="w-100 p-4 border rounded" style={{ fontSize: "23px" }}>
        <div className="row">
          ...
          <div className="col-12">
            ...
            <div className="d-flex flex-wrap align-items-center gap-2">
              <input
                ...
                style={{ width: "450px", ...inputStyle }}
                value={selectedProductType}
                readOnly
              />
              {renderCropSelect(fruits, "فواكه...")}
              {renderCropSelect(vegetables, "خضروات...")}
              {renderCropSelect(seeds, "حبوب...")}
            </div>
          </div>
        </div>
        ...
        <div className="row">
          <div className="col-md-6">
            ...
            <select
              ...
              style={inputStyle}
              disabled={!isOpenForInvestmentState}
            >
