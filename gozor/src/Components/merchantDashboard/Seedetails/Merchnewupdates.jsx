...
export default function Invnewupdates({ cycleId, cycleUpdates }) {
    const [cycles, setCycles] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);

    // Fetch cycle data (now receives data through props)
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                if (cycleUpdates) {
                    // Data is provided through prop
                    setCycles(cycleUpdates);
                } else if (cycleId) {
                    // Fetch data using cycleId
                    const response = await axios.get(
                        `https://cityroots.runasp.net/api/CycleUpdate/cycle/${cycleId}`
                    );
    ...
    const formatDate = (dateString) => {
        ...
        return date.toLocaleDateString('en-US'); // Or your desired locale
    };
                                            ...
                                            <div className="row mb-3">
                                                ...
                                                <div className="col-md-6">
                                                    ...
                                                    <input style={{ borderRadius: "20px" }}
                                                        type="text"
                                                        className="form-control"
                                                        value={formatDate(cycle.updateDate)}
                                                        readOnly
                                                    />
                                                </div>
                                            </div>
